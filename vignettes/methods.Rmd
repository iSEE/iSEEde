...
Because the output of `limma::topTable()` is a standard `data.frame`, it must first be manually passed to the `iSEELimmaResults()` constructor, to identify it as such.
The constructor is also given the full vector of rownames in the `airway` object, so that the `iSEELimmaResults` object matches the dimensions of the object in which it is to be embedded.

This manual method is preferable to any heuristic that would attempt to use the column names of the `data.frame` for identifying it as a `limma::topTable()` output.
...
library(iSEEde)
airway <- embedResults(
  x = iSEELimmaResults(res_limma, row.names = rownames(airway)),
  se = airway,
  name = "Limma-Voom")
rowData(airway)
```
...
Again, we embed this set of differential expression results in the `airway` object using the `embedResults()` method.
