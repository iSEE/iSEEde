...
rowData(airway)[["iSEEde"]] <- DataFrame(
  "Limma-Voom" = I(iSEELimmaResults(tt, row.names = rownames(airway))),
  check.names = FALSE
)
```
...
dds <- DESeq(dds)
res <- results(dds, contrast = list("dextrt", "dexuntrt"))
head(res)
```
...
```{r}
rowData(airway)[["iSEEde"]][["DESeq2"]] <- iSEEDESeq2Results(res, row.names = rownames(airway))
```
...
SCREENSHOT("screenshots/supported_methods.png", delay=20)
```
