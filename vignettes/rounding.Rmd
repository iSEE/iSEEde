---
title: "Rounding numerical values"
author: 
...
vignette: >
  %\VignetteIndexEntry{Rounding numerical values}
...
Differential expression methods generally return precise numeric values with many digits after the decimal point.
...
The default rounding configuration can be overridden in individual panel configuration.

The slots `RoundDigits` and `SignifDigits` can be set directly in the individual calls to the `DETable()` constructor function.

In the example below, we add two tables, one rounding numeric values to two significant digits, the other rounding the same values to three significant digits.

```{r, message=FALSE}
library(iSEE)
app <- iSEE(airway, initial = list(
  DETable(ContrastName="edgeR", HiddenColumns = c("logCPM", "LR"),
          PanelWidth = 6L, RoundDigits = TRUE, SignifDigits = 2L),
  DETable(ContrastName="edgeR", HiddenColumns = c("logCPM", "LR"),
